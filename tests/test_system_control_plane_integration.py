...
def test_control_plane_scripts_compose_end_to_end(tmp_path: Path) -> None:
    ...
    startup_result = _run_script(
        ...
        "--product-stage-now",
        "Pre-flight kernel integration",
        "--product-stage-intent",
        "Product-directed startup for the control-plane loop",
        "--product-stage-out-of-scope",
        "Real-use rollout validation and final cleanup",
        "--product-problem-this-round",
        "The startup layer must carry product-stage and surface-lifecycle intent before execution.",
        "--why-now",
        "This integration test should exercise the same product-aware startup path as live rounds.",
        "--if-we-skip-this",
        "The loop can pass mechanically while still starting without explicit product framing.",
        ...
        "--mvp-next-stage-gate",
        "Startup emits product-aware artifacts before worker execution.",
        "--next-simplification-step",
        "Collapse repeated kickoff framing into canonical product-stage fields.",
        "--positioning-lock",
