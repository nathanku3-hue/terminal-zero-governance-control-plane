...
| C1.3 | CODEOWNERS confirmation | DONE | Confirm maintainer handle OR add follow-up note per `RELEASING.md:122` | @nathanku3-hue in CODEOWNERS |
...
| C1.7 | Tests pass | DONE | Run `python -m pytest -q` | 473 passed, 33 skipped (60s) |
| C1.8 | CLI smoke | DONE | Run `sop --help && sop version && sop init /tmp/smoke-test` | sop --help ✓, sop version (0.1.0) ✓, sop init ✓ |
...
| C3.5 | Restore real phase_end_handover test | Low | Replace skipped real-contract test (ada0297) with non-flaky variant | Coverage reduction in v0.1.0 acceptable but not permanent |
